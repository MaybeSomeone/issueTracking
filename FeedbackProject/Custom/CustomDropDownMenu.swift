...
//  FeedbackProject
//
//  Created by Currie.shen on 2022/2/28.
//
...
public protocol CustomDropDownMenuDataSource: AnyObject {
    func numberOfRows(in column: NSInteger, for menu: CustomDropDownMenu) -> Int
    func titleForRow(at indexPath: CustomIndexPath, for menu: CustomDropDownMenu) -> String
    func numberOfColumns(in menu: CustomDropDownMenu) -> NSInteger
    func titleFor(column: Int, menu: CustomDropDownMenu) -> String
   
}

public extension CustomDropDownMenuDataSource {
    func numberOfColumns(in menu: CustomDropDownMenu) -> NSInteger {
        return 1
    }
    
    func titleFor(column: Int, menu: CustomDropDownMenu) -> String {
        return menu.datasource?.titleForRow(at: CustomIndexPath(column: column, row: 0), for: menu) ?? ""
    }
}

public protocol CustomDropDownMenuDelegate: AnyObject {
    func willSelectClickedRow(at index: Int, for menu: CustomDropDownMenu)
    func didSelectRow(at indexPath: CustomIndexPath, for menu: CustomDropDownMenu)
}

public class CustomDropDownMenu: UIView {
    ...
    open var textFont = UIFont.systemFont(ofSize: CGFloat(12.0))
    open var updateColumnTitleOnSelection = true
    open var arrowPostion: ArrowPosition = .Right
    open weak var datasource: CustomDropDownMenuDataSource? {
        didSet {
            //configure view
    ...
    open weak var delegate: CustomDropDownMenuDelegate?
    func setUpUI() {
    ...
    func calculateTitleSizeWith(string: String) -> CGSize {
        let dict = [NSAttributedString.Key.font: textFont]
...
extension CustomDropDownMenu {
    ...
    @objc func menuTapped(paramSender: UITapGestureRecognizer) {
        ...
        self.delegate?.willSelectClickedRow(at: tapIndex, for: self)
    }
...
extension CustomDropDownMenu: UITableViewDataSource, UITableViewDelegate {
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell?.textLabel?.text = self.datasource?.titleForRow(at: CustomIndexPath(column: self.currentSelectedMenuIndex, row: indexPath.row), for: self)
    ...
    public func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        self.delegate?.didSelectRow(at: CustomIndexPath(column: self.currentSelectedMenuIndex, row: indexPath.row), for: self)
    }

    func confiMenuWith(row: NSInteger) {
        let title = self.titles[self.currentSelectedMenuIndex]
        if updateColumnTitleOnSelection {
            title.string = self.datasource?.titleForRow(at: CustomIndexPath(column: self.currentSelectedMenuIndex, row: row), for: self)
        }
