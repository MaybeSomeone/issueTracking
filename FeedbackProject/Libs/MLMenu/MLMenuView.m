...
- (void)delaDataWithTitles:(NSArray *)titles WithImageNames:(NSArray *)imageNames
{
    ...
    for (NSString *title in titles) {
        ...
        item.separatorColor = MLClolor(255, 255, 255, 1);
...
{
    self.backgroundColor = [UIColor clearColor];
    self.tableView.backgroundColor = backgroundColor != nil ? self.tableView.backgroundColor = backgroundColor : MLClolor(0, 0, 0, 1);;
}
...
- (void)drawRectCoverViewTriangleOffset:(CGFloat)offset
{
    ...
    CAShapeLayer *layer =[CAShapeLayer layer];
    layer.path = path.CGPath;
    [layer setFillColor:MLClolor(0, 124, 194, 1).CGColor];
